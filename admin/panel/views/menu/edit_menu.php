...
<form id="form" action="<?=current_url()?>/" method="post">
    ...
    <div id="fields_with_errors">
       <input name="menu[menu_name]" maxlength="40" type="text"  value="<?=$data[0]['menu_name']?>" onblur="if (this.value=='') this.value='Введите название меню';" onfocus="if (this.value=='Введите название меню') this.value='';"/>
    ...
    <div id="fields_with_errors2">
        <select name="connection[position_id]">
            ...
            <? $selected = ($data[0]['position_id'] == $position['id']) ? 'selected' : '' ?>
    ...
    <input type="hidden" name="menu[published]" value="<?=$data[0]['published']?>" />
</form>