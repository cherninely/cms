...
//            $this->_ajax_controller(array('type' => 'search', 'search_text' => 'а'));
            if($_POST){
                $this->_ajax_controller($_POST);
            }
            
            if(!$this->uri->segment(1, 0)){ //Главная
               ...
               $this->load->model('mdl_loader');
                ...
                $modules_per_page = $this->mdl_loader->get_modules_per_page($this->uri->segment(1)); //получение модулей для страницы
                ...
                foreach($modules_per_page as $module){
                    ...
                    $data[$module['position_name']] .= $this->$module['module_type']->$module_number($module,$this->uri->segment(1),$this->uri->ruri_to_assoc());//подгружаем библиотеки (модули)
                    $data['header'] .= $this->$module['module_type']->load_files();

                }

                    
                $is_module = $this->mdl_loader->is_module($this->uri->segment(1)); 
                if($is_module){//если это страница с компонентом
                     ...
                     $data['content'] .= $this->$module_type[0]['module_type']->core1($this->uri->segment(1),$this->uri->ruri_to_assoc(4));
